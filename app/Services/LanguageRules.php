...
class LanguageRules
{
    ...
    public function setLocales(?array $locales = null): self
    {
        ...
        $langs = app()->languages->all()->slug();
        $default= config('app.locale');

        if(isset(request()->enable) && !ArrayHelper::cnt(request()->enable, 0)) {

            // массив влюченных языковых версий
            $enable = array_keys(request()->enable);

            // удаляем из массива языков, язык что не входит в массив влюченных языковых версий
            // и не является языком по умолчанию
            foreach ($langs as $k=>$lang){

                if($lang !== $default && !in_array($lang, $enable) ){
                    unset($langs[$k]);
                }
            }
