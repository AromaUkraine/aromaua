...
@section('actions')
    <x-action
        href="{{ route('module.page_banner.create', [$page->id, $pageComponent->alias]) }}"
    ...
    @if(!request('trash'))
        <x-action
            href="{{ route('module.page_banner.index', [$page->id, $pageComponent->alias, 'trash'=>true]) }}"
            class="danger text-capitalize "
            title="{{__('cms.buttons.deleted')}}"
            icon="bx bx-trash"></x-action>
    @else
        <x-action
            href="{{ route('module.page_banner.index', [$page->id, $pageComponent->alias]) }}"
