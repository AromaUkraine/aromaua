...
@section('actions')
    ...
    @if(!request('trash'))
        ...
        <x-action
            href="{{ route('module.gallery.create', [ 'table'=>$table,'id'=>$id,'type'=>\Modules\Gallery\Entities\Gallery::TYPE_VIDEO ]) }}"
            class="primary text-capitalize  mr-1"
            title="{{__('cms.buttons.add_video')}}"
            icon="bx bx-plus"></x-action>

        <x-action
            href="{{ route('module.gallery.create', ['table'=>$table,'id'=>$id,'type'=>\Modules\Gallery\Entities\Gallery::TYPE_PHOTO]) }}"
            class="success text-capitalize  mr-1"
            title="{{__('cms.buttons.add_photo')}}"
            icon="bx bx-plus"></x-action>

        <x-action
            href="{{ route('module.gallery.index', [ $table, $id, 'trash'=>true]) }}"
