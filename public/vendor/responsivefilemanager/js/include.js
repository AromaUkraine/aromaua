...
(function ($, Modernizr, image_editor) {
    ...
    /*** custom button apply ***/
    var parentWindowButton =  jQuery('button.apply', window.parent.document);

    if(typeof parentWindowButton !== typeof undefined){
        parentWindowButton.hide();
    }
    /*** end custom button apply ****/
    ...
    jQuery(document).ready(function () {
        ...
        jQuery('.ff-container').on('click', '.checkmark', function (e) {
            ...
            /**** toggle custom button apply ****/
            (checked > 0) ? parentWindowButton.show() : parentWindowButton.hide();
        ...
        /*** custom button apply ***/
        parentWindowButton.on('click', function (e) {
            var files = getFiles();
            window['apply'](files, jQuery('#field_id').val());
        })
    ...
    apply = function (files, external) {
        ...
        var callback = jQuery('#callback#callback').val();
        ...
        if (external != "") {
            ...
            if (jQuery('#crossdomain').val() == 1) {
                ...
                if(typeof parent.tinymce !== typeof undefined){
    ...
    function close_window()
    {
        ...
        if (jQuery('#popup').val() == 1) {
            ...
            /*** custom hide modal ***/
            if (typeof parent.jQuery('.modal:has(object)').modal == "function") {
                parent.jQuery(".modal:has(object)").modal("hide");
                parent.jQuery(document.body).removeClass('modal-open');
