(function (factory) {
    ...
    $.jstree.plugins.actions = function (options, parent) {
        ...
        this.add_action = function (node_id, action ) {
            ...
            action = typeof action === 'object' ? action : typeof action === 'function' ? action() : [];
        ...
        this._create_action = function (node_id, action_id , obj, el) {
            ...
            var node  = self.get_node(obj);
            // console.log(el);
            // console.log(obj);


            var action_el = document.createElement(action.selector);

            // console.log(node.original.button_controls);

            if(node.original.button_controls[action_id]) {

                $(action_el).attr('href', node.original.button_controls[action_id]);
                action_el.className = action.class;

                // console.log(node.original.button_controls[action_id]);
                // console.log(action_id);

                // $.each(node.original.button_controls , function (val, key){
                //     console.log('button key', key);
                //     console.log('button value', value);
                //     if(key === action_id) {
                //
                //         console.log('create button', key);
                //     }
                //
                // });

                // $.each(node.li_attr , function (val, key){
                //     console.log('val', val);
                //     console.log('key', key);
                // })
                ...
                if( action.classByCondition ){

                    var attr = action.classByCondition.attr;
                    var additionalClass = action.classByCondition.class;
                    var key = node.a_attr[attr];
                    action_el.className += " "+additionalClass[key];
                }

                action_el.textContent = action.text;

                if($.type( action.title ) === 'string' && action.title.length > 0){
                    action_el.title = action.title;
                }else if($.type( action.title ) === 'object'){
                    action_el.title = action.title[key];
                }


                if( action.attributes !== undefined){
                    $.each(action.attributes, function (k, v) {
                        $(action_el).attr(k,v);
                    })
                }

                let item =  $('body').find('div.tooltip');
                if(item.length){
                    item.map( function (k, el) {
                        $(el).remove();
                    })
                }
        ...
        this._set_action = function (node_id, obj, action) {
            if (action === null || typeof action === typeof undefined) return;
        ...
        this.redraw_node = function (obj, deep, callback, force_draw) {
            ...
            if (el) {
                ...
                for (var i = 0; i < actions.length; i++) {
                    var _action = self._create_action(node_id, actions[i].id, obj, el);
                ...
                for (i = 0; i < actions.length; i++) {
                    _action = self._create_action("all", actions[i].id, obj, el);
                    self._set_action(node_id, el, _action);
                }
            }
            return el;
